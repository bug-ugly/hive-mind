public class DeepQNetwork {
  ...
  DeepQNetwork(int [] layers, int replayMemoryCapacity, float discount, 
    ...
    DeepQ = new NeuralNetwork(layers ); //initializing two networks because in deep Q learning there are 2 feed forw steps
    TargetDeepQ = new NeuralNetwork(layers);
  ...
  void AddReplay(float reward, float[] NextInput, int NextActionMask[]) {
    if ( ReplayMemory.size() >= ReplayMemoryCapacity )
      ReplayMemory.remove( int(random(ReplayMemory.size())) );
  ...
  Replay[] GetMiniBatch(int BatchSize) {
    ...
    for (int i = 0; i < size; i++) {
      retVal[i] = ReplayMemory.get(int (random(ReplayMemory.size())));
    }
    return retVal;
  }
  ...
  int GetAction(float[] Inputs, int ActionMask[]) {
    ...
    if (Epsilon < r) {
      LastAction = int(random(outputs.length));
  ...
  void TrainNetwork(int BatchSize) {
    ...
    float[][] CurrOutputs = new float [CurrInputs.length] [CurrInputs[0].length];
    float[][] TargetOutputs = new float [TargetInputs.length] [TargetInputs[0].length];

    for ( int row = 0; row<CurrOutputs.length; row++) {

      float [] row1 = DeepQ.FeedForward(getaRow(CurrInputs, row));
      float [] row2 = TargetDeepQ.FeedForward(getaRow(TargetInputs, row));
    ...
    for (int i = 0; i < y.length; i++) {
      ...
      if (replays[i].NextInput != null)
        ...
        FutureReward = FindMax(getaRow(TargetOutputs, i), replays[i].NextActionMask);
    ...
    //println("Avgerage Error: " + (TotalError / y.length) );
